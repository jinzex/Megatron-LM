...
| Component | Fix | Patch |
|-----------|-----|-------|
| **TP-Invariant GEMM** (fwd+bwd) | All-gather sharded weight, full-K GEMM | `patches/layernorm_linear.py`, `patches/linear.py` |
| **Gated deinterleave** (bwd) | Reorder via `partition_stride` after all-gather | `patches/layernorm_linear.py` |
| **Cross-entropy** (fwd) | All-gather exp_logits, local sum | `patches/cross_entropy.py` |
| **Output projection** (bwd) | All-gather weight+grad, full dgrad GEMM | `patches/layers.py` |
| **Gradient clipping** | Float64 norm + pow2 clip_coeff rounding | `megatron/core/optimizer/clip_grads.py` |
| **RMSNorm dgamma** (bwd) | All-gather tokens + rank-0-only reduction | `megatron/core/.../batch_invariant_kernels.py` |
| **BIK** (fwd+bwd) | M-invariant Triton matmul_persistent | `megatron/core/.../batch_invariant_kernels.py` |
...
**Prerequisites:** TE 2.9, 8x H100, this Megatron-LM branch on PYTHONPATH.

### Install TE patches

```bash
PROJ=<path-to-repo>
PATCHES=$PROJ/examples/tp-numerics/patches
...
cp $PATCHES/layernorm_linear.py $TE_MOD/ && cp $PATCHES/linear.py $TE_MOD/
cp $PATCHES/backends.py $TE_DPA/ && cp $PATCHES/dot_product_attention.py $TE_DPA/
cp $PATCHES/cross_entropy.py $PROJ/megatron/core/tensor_parallel/
cp $PATCHES/layers.py $PROJ/megatron/core/tensor_parallel/
...
SEQUENCE_PARALLEL=1 TEST_BACKWARD=1 \
torchrun --nproc_per_node=8 examples/tp-numerics/test_tp_numerics.py

# Unit test: MoE (requires BIK=1)
PYTHONPATH=. NVTE_ALLOW_NONDETERMINISTIC_ALGO=0 NVTE_TP_INVARIANT_MODE=1 \
SEQUENCE_PARALLEL=1 TEST_BACKWARD=1 MOE=1 BIK=1 \
torchrun --nproc_per_node=8 examples/tp-numerics/test_tp_numerics.py

# E2E: Dense Qwen3-0.6B (100 iters, TP=1 vs TP=2 vs TP=4)
...
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True BIK=1 ATTN_BACKEND=unfused

for TP in 1 2 4; do
    TP_SIZE=$TP TRAIN_ITERS=100 torchrun --nproc_per_node=$TP \
      examples/tp-numerics/validate_e2e_dense_qwen3_0.6b.py \
      2>&1 | grep "iteration.*lm loss" | tee /tmp/tp${TP}.log
done

# Verify (empty diff = bitwise identical):
