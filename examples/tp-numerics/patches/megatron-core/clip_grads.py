...
def clip_grad_by_total_norm_fp32(
    ...
    # TP-invariant clip_coeff: round to nearest power of 2 (zero mantissa bits)
    # so the tiny float64 inter-TP norm difference (~6e-7) can't produce different
    # clip_coeffs. The exact clip value doesn't affect training — only TP-invariance.
    #
    ...
    if _clip_os.environ.get("NVTE_TP_INVARIANT_MODE", "0") == "1":
        import math
        if clip_coeff > 0:
