...
#SBATCH --job-name=tp-inv-30b
#SBATCH --output=slurm_%j.log
#SBATCH --exclusive

# ==============================================================================
# E2E TP-Invariant Validation — Qwen3-30B-A3B MoE (4 nodes)
#
# Full-scale 30B MoE (48 layers, 128 experts, top-8) stress test.
# Validates TP-invariant mode at production scale.
#
# Usage:
#   TP_SIZE=1 sbatch submit_e2e_qwen3_30b.sh    # baseline
#   TP_SIZE=2 sbatch submit_e2e_qwen3_30b.sh
#   TP_SIZE=4 sbatch submit_e2e_qwen3_30b.sh
# ==============================================================================
...
WANDB_EXP_NAME="tp-inv-qwen3-30b-tp${TP_SIZE}"
...
###############################################################################
# Auto-build Megatron-LM C++ extensions if needed
###############################################################################

HELPERS_SO_DIR="${MEGATRON_LM_PATH}/megatron/core/datasets"

if ! ls ${HELPERS_SO_DIR}/helpers_cpp*.so 1>/dev/null 2>&1; then
    echo "Building Megatron-LM C++ extensions..."
    srun --ntasks=1 --gpus-per-node=1 \
        --no-container-mount-home \
        --container-image=${CONTAINER_IMAGE} \
        --container-mounts=${CONTAINER_MOUNTS} \
        --container-workdir=${MEGATRON_LM_PATH} \
        bash -c "pip install -e . --no-deps --no-build-isolation"
fi
...
sys.path.insert(0, os.environ["MEGATRON_BRIDGE_PATH"] + "/scripts/performance")
...
config.model.recompute_modules = ["core_attn", "moe_act", "layernorm"]
...
echo "======================================"
echo "TP-Invariant E2E: Qwen3-30B-A3B"
...
srun \
    ...
    bash -c "
        ...
        numactl --cpunodebind=\$((SLURM_LOCALID/${NUMA_DIVISOR})) \
